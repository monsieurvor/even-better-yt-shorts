...
const getCurrentId = () => {
  const videoEle = document.querySelector("ytd-reel-video-renderer[is-active='']");
  if (videoEle) return videoEle.id;
  return null;
};
...
const setActionList = (currentId, actionList) => {
  ...
  injectedItems.add(currentId);

  return injectedSuccess
};

const setActionListFuncs = (ytShorts) => {
...
const setProgressBar = (overlayList) => {
  ...
  return (progBarList, timestampTooltip);
};

const setProgressBarFuncs = (ytShorts, progBarList, timestampTooltip) => {
...
const timer = setInterval(() => {
  ...
  if (injectedItems.has(currentId)) {
    ...
    if (currTime !== lastTime) {
      ...
      if (!injectedSuccess) {
        injectedItems.delete(currentId) 
      } else {
        lastTime = currTime;
      };
    }
    ...
    if (actionList) {
      injectedSuccess = setActionList(currentId, actionList);
      setActionListFuncs(ytShorts);
    }
    if (overlayList) {
      let (progBarList, timestampTooltip) = setProgressBar(overlayList);
      setProgressBarFuncs(ytShorts, progBarList, timestampTooltip);
    }
